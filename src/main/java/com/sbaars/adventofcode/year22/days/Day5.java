...
//    d.submitPart1();
  ...
  private static List<Deque<Integer>> input() {
    ...
    for(int i = 1; i<=9; i++){
      ...
      switch (i) {
        case 1 -> "NSDCVQT".chars().forEach(s::add);
        case 2 -> "MFV".chars().forEach(s::add);
        case 3 -> "FQWDPNHM".chars().forEach(s::add);
        case 4 -> "DQRTF".chars().forEach(s::add);
        case 5 -> "RFMNQHVB".chars().forEach(s::add);
        case 6 -> "CFGNPWQ".chars().forEach(s::add);
        case 7 -> "WFRLCT".chars().forEach(s::add);
        case 8 -> "TZNS".chars().forEach(s::add);
        case 9 -> "MSDJRQHN".chars().forEach(s::add);
      }
      stacks.add(s);
    }
    return stacks;
